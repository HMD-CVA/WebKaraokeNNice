...
app.get('/admin/phonghat', async (req, res) => {
    try {
        const [phonghats, bangGiaList, roomTypes] = await Promise.all([DataModel.Data_PhongHat_Model.find({}).lean().exec(), DataModel.Data_BangGiaPhong_Model.find({}).lean().exec(), DataModel.Data_BangGiaPhong_Model.distinct('LoaiPhong')])

        // Tạo map để tra cứu nhanh bảng giá theo LoaiPhong
        const phonghatsWithPrice = phonghats.map((phong) => {
            const bangGiaCungLoai = bangGiaList.filter((banggia) => banggia.LoaiPhong === phong.LoaiPhong)
...
app.get('/admin/thietbi', paginationMiddleware(DataModel.Data_ThietBi_Model), async (req, res) => {
    try {
        let stats = await DataModel.Data_ThietBi_Model.aggregate([
            {
                $group: {
                    _id: '$TinhTrang',
                    SoLuong: { $sum: 1 },
        ...
        const iconMap = {
            'Cần bảo trì': 'fas fa-exclamation-triangle',
            'Đang bảo trì': 'fas fa-tools',
            'Đang sửa chữa': 'fas fa-tools',
            Tốt: 'fas fa-check-circle',
            Hỏng: 'fas fa-exclamation-triangle',
            Total: 'fas fa-microphone',
        }

        const bgColor = {
            'Cần bảo trì': 'stat-icon service',
            'Đang bảo trì': 'stat-icon reception',
            'Đang sửa chữa': 'stat-icon reception',
            Tốt: 'stat-icon active',
            Hỏng: 'stat-icon service',
            Total: 'stat-icon total',
        }

        const thietbisThongKe = stats.map((item) => {
            return {
                ...item,
                icon: iconMap[item.TinhTrang] || iconMap['Total'],
                bgColor: bgColor[item.TinhTrang] || bgColor['Total'],
            }
        })

        thietbisThongKe.unshift({
            SoLuong: res.totalItems,
            TinhTrang: 'Tổng thiết bị',
            icon: iconMap['Total'],
            bgColor: bgColor['Total'],
        })

        const thietbis = res.items
...
// Quản lý nhân viên
app.get('/admin/nhanvien', paginationMiddleware(DataModel.Data_NhanVien_Model), async (req, res, next) => {
    try {
        let stats = await DataModel.Data_NhanVien_Model.aggregate([
            {
                $group: {
                    _id: '$VaiTro', // 1. Nhóm theo VaiTro
                    SoLuong: { $sum: 1 },
                },
            },
            {
                $project: {
                    _id: 0, // Ẩn _id
                    VaiTro: '$_id', // 2. Đổi tên _id thành 'VaiTro' (cho nhất quán)
                    SoLuong: 1,
                },
            },
            {
                $sort: { VaiTro: 1 }, // 3. Sắp xếp theo 'VaiTro'
            },
        ])

        const iconMap = {
            'Quản lý': 'fas fa-user-tie',
            'Phục vụ': 'fas fa-utensils',
            'Lễ tân': 'fas fa-concierge-bell',
            'Kỹ thuật': 'fas fa-tools',
            'Bảo vệ': 'fas fa-shield-alt',
            Total: 'fas fa-user',
        }

        const bgColor = {
            'Quản lý': 'stat-icon manager',
            'Phục vụ': 'stat-icon service',
            'Lễ tân': 'stat-icon reception',
            'Kỹ thuật': 'stat-icon technical',
            'Bảo vệ': 'stat-icon security',
            Total: 'stat-icon total',
        }

        const nhanviensThongKe = stats.map((item) => {
            return {
                ...item,
                icon: iconMap[item.VaiTro] || iconMap['Total'],
                bgColor: bgColor[item.VaiTro] || bgColor['Total'],
            }
        })

        nhanviensThongKe.unshift({  
            SoLuong: res.totalItems,
            VaiTro: 'Tổng nhân viên',
            icon: iconMap['Total'],
            bgColor: bgColor['Total'],
        })
...
// app.get('/admin/nhanvien', async (req, res, next) => {
//     try {
...
//         const totalItems = await DataModel.Data_NhanVien_Model.countDocuments()
