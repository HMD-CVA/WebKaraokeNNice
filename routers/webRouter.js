...
router.get('/logout', (req, res, next) => {
    req.logout(function (err) {
        ...
        req.session.destroy(err => {
            if(err) return res.status(500).json({message: 'Loi server', error: err.message})
...
router.get('/profile', checkLogin, async (req, res, next) => {
    try {   
        ...
        if (!id) return res.json('Chua dang nhap')
        const user = await DataModel.Data_ThongTinTaiKhoan_Model.findOne({TaiKhoanId: id}).populate('TaiKhoanId')
        if (!user) return res.json('Chua dang nhap')
        res.json(user)
    } catch (error) {
